(() => {
  ...
  let saved = {};
  try {
    saved = JSON.parse(localStorage.getItem("ln_settings") || "{}");
  ...
  // Shared helpers
  function attachSources(container, arr) {
    if (!container) return;
    container.innerHTML = "";
    (arr || []).forEach((s) => {
      ...
      const chipLabel =
        `[${s.rank}] ${s.title || ""}` + (s.heading ? ` — ${s.heading}` : "");
      ...
      chip.addEventListener("click", () =>
        toggleSnippet(container.closest(".bubble"), s),
      );
      container.appendChild(chip);
    });
  }

  function createAssistantBubble(agentMode) {
    ...
    return { assistantBubble, typing, traceContainer, toolsRow, thinkingLines };
  }
  ...
  function wireAgentEvents(
    ...
    es.addEventListener("sources", (ev) => {
      if (!agentMode) {
        try {
          attachSources(srcContainer, JSON.parse(ev.data || "[]"));
        } catch {}
      }
    });
    ...
    if (agentMode) {
      es.addEventListener("tool", (ev) => {
        try {
          ...
          if (traceContainer) {
            ...
            line.textContent =
              `Tool: ${d.name || ""} ${d.args ? JSON.stringify(d.args) : ""}`.trim();
      ...
      es.addEventListener("thinking", (ev) => {
        ...
        if (payload && payload.id != null) {
          ...
          if (target) target.textContent += payload.text || "";
        }
      });
      es.addEventListener("thinking_end", (ev) => {
        ...
        const el =
          traceContainer &&
          traceContainer.querySelector(`.thinking-line[data-id="${id}"]`);
        if (el) {
  ...
  ovCancel &&
    ovCancel.addEventListener("click", () => {
      ...
      ovCancel.classList.add("hidden");
      ovClose.classList.remove("hidden");
    });
  ovClose &&
    ovClose.addEventListener("click", () => {
      closeOverlay();
      ovClose.classList.add("hidden");
      ovCancel.classList.remove("hidden");
    });

  function resetOverlay() {
    ovScan.max = 0;
    ovScan.value = 0;
    ovScanTxt.textContent = "";
    ovPlan.max = 1;
    ovPlan.value = 0;
    ovPlanTxt.textContent = "";
    ovFetch.max = 1;
    ovFetch.value = 0;
    ovFetchTxt.textContent = "";
    ovEmbed.removeAttribute("max");
    ovEmbed.value = 0;
    ovEmbedTxt.textContent = "";
    ovClose.classList.add("hidden");
    ovCancel.classList.remove("hidden");
  }
  function appendMessage(role, content, renderMd = true) {
    ...
    if (role === "assistant") {
      ...
      copyBtn.addEventListener("click", async () => {
        ...
        try {
          await navigator.clipboard.writeText(md ? md.innerText : "");
          copyBtn.textContent = "Copied";
          setTimeout(() => (copyBtn.textContent = "Copy Answer"), 1000);
        } catch {}
      });
  ...
  function startIndexing() {
    ...
    indexStream.addEventListener("scan", (ev) => {
      try {
        const d = JSON.parse(ev.data || "{}");
        ovScan.max = d.total || 0;
        ovScan.value = 0;
        ovScanTxt.textContent = `${ovScan.value}/${ovScan.max}`;
      } catch {}
    });
    indexStream.addEventListener("plan", (ev) => {
      try {
        const d = JSON.parse(ev.data || "{}");
        ovPlan.value = 1;
        ovPlanTxt.textContent = `${d.changed || 0} changed`;
      } catch {}
    });
    indexStream.addEventListener("fetch", (ev) => {
      try {
        const d = JSON.parse(ev.data || "{}");
        ovFetch.value = 1;
        ovFetchTxt.textContent = `${d.got || 0} bodies`;
      } catch {}
    });
    indexStream.addEventListener("note", (ev) => {
      // advance scan if known max
      if (ovScan.max > 0) {
        ovScan.value = Math.min(ovScan.max, ovScan.value + 1);
        ovScanTxt.textContent = `${ovScan.value}/${ovScan.max}`;
      }
    });
    indexStream.addEventListener("embed", (ev) => {
      try {
        const d = JSON.parse(ev.data || "{}");
        ovEmbed.value += d.inc || 1;
        ovEmbedTxt.textContent = `${ovEmbed.value} chunks`;
      } catch {
        ovEmbed.value += 1;
      }
    });
  ...
  function saveSettings() {
    const s = {
      ...
      recency: Number(recencyEl.value || "10"),
      agentMode: agentModeEl ? !!agentModeEl.checked : false,
    };
    localStorage.setItem("ln_settings", JSON.stringify(s));
  }
  ...
  // Top-level helpers used by streaming renderers
  ...
  async function listConversations() {
    ...
    items.forEach((c) => {
      ...
      del.onclick = async (e) => {
        ...
        if (arr.length) selectConversation(arr[0].id);
        else {
          currentConv = null;
  ...
  function startEditMessage(bubble, m) {
    ...
    save.addEventListener("click", async () => {
      await fetch(`/conv/${currentConv}/message/${m.id}`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ content: editor.value }),
      });
      await fetch(`/conv/${currentConv}/truncate`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ upto_id: m.id }),
      });
  ...
  async function runFromHere(question) {
    ...
    if (agentMode) {
      ...
      body = {
        ...
        provider: providerEl.value || "ollama",
        llm_model: llmModelEl.value || null,
        recency_alpha: Math.max(
          0,
          Math.min(1, Number(recencyEl.value || "10") / 100),
        ),
      };
    ...
    // Wire SSE events using shared helper
    wireAgentEvents(
      es,
      assistantBubble,
      typing,
  ...
  async function send() {
    ...
    // Reuse shared wiring for SSE events
    wireAgentEvents(
      es,
      assistantBubble,
      typing,
  ...
  function toggleSnippet(bubble, source) {
    ...
    const updated = source.updated_at
      ? new Date(source.updated_at * 1000)
      : null;
